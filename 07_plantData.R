...
mod2 =
  nlme::lme(CFUxg ~ 0 + flhc + flhc:AMF, random = ~ 1 | strain,
            weight = nlme::varIdent(form = ~ 1 |  strain * AMF),
            data = colonization_bac)
...
df_mod =
  marginaleffects::avg_predictions(mod2, variables = c("flhc", "AMF"), hypothesis = difference ~ pairwise) |>
  tidy() |>
...
p1 =
  colonization_bac |>
  ggplot(aes(x = flhc_AMF, y = CFUxg, color = flhc_AMF, shape = strain,
  ...
  geomtextpath::geom_textsegment(data = df_mod |>
                                   filter(row_number() %in% c(1, 2, 5, 6)),
                                 aes(x = start,
                                     xend = end,
                                     y = pos_y,
                                     yend = pos_y,
                                     label = format(p.adj, scientific = T, digits = 2)), inherit.aes = F,
                                 size = 12/.pt)
...
mod1 =
  ...
  nest(.by = name) |>
  mutate(fit = map(data, ~ nlme::lme(value ~ flhc, random = ~ 1 | strain,
...
p2 = colonization_AMF |>
  pivot_longer(Vesicle:Hyphae) |>
  ggplot(aes(x = flhc, y = value, color = flhc, shape = strain,
             fill = after_scale(color))) +
  geom_point(position = position_dodge(width = .5), size = 3, alpha = .8) +
  stat_summary(
    geom = "pointrange",
  ...
  scale_x_discrete(labels = list("flhC+" = "*flhC*+",
                                 "flhC-" = "*flhC*-")) +
